...
For the sake of introduction and familiarizing ourselves with R and RStudio, we will limit ourselves only to calculating and pooling the effect sizes of event rate in a single arm trial, using a dummy dataset. 
...
Meta-analysis is a quantitative summary of a given data. It is widely used in health care sciences to summarize the effects and outcomes of an intervention which is usually spread across multiple trials. In addition to the qualitative analysis which mostly is done in the form of systematic reviews and focused reviews, meta-analysis helps us in quantifying the net effect spread across all those bits of data. 
The subject of meta-analysis is quite extensive and a person learns this process gradually overtime. One of the very good resources to understand the basics of meta-analysis is the book, "Introduction to Meta-analysis," by Borenstein et al. (2011). 
...
In order to use a package, we have to load a library first. 
...
After seeing the documentation, you will be able to have a basic idea about the basic functions and their uses. 
...
After installing and loading the package, let us take a simple example of conducting meta-analysis in R using an example dataset of single arm trial reporting a Y event rate for a drug X. In this article, we will only calculate the effect sizes and generate a default forest plot.
...
## Metaprop Function
Metaprop is a function in the meta package which can be used to pool the effect sizes of proportions from a single arm study. You will find more information about other functions from the meta documentations.

## Code for meta-analysis
The code for meta-analysis using metaprop function taking into account various variable arguments is below and we will assign the results of our meta-analysis to another variable, let's call it def.
...
Firstly we have assigned the appropriate column so that it can get the data. event=event.e means that for the event, the data is in the column event.e of our dataset, and the dataset is abc. And the study labels are in column Study. 
sm indicates which summary measure should be used to pool effect sizes. Here we have used Plogit. Some other commonly used measures like Freeman Tukey Double Arscine Transformation are written along with their respective codes in documents. 
Level stands for confidence level which we have set to 95%. comb.fixed and comb.random stands for fixed and random effects meta-analysis which we have set to true as we want to see the results from both of them. method.tau is the method used to calculate between study variance and the most commonly used is Dersimonian Laird which we have used. 

There are a ton of other arguments but for the beginning, we will only use these ones and leave others to the default settings. 

Now, in order to take a look at the results of the meta-analysis, just write the value which you have assigned to the results of the meta-anlaysis and run it using ctrl+enter. And it will give the details of meta-analysis including various statistical results. Here our command would be:
...
You will also find the values of Tau^2, Q statistic, I^2, which will help in identifying the heterogeneity in the studies.
...
## Conclusion
We have successfully generated the forest plot of our studies. ALthough a single arm meta-analysis is not commonly used, but we have used it to orient ourselves to the working of R and basic methodology for a meta-analysis in this language. 
